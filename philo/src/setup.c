...
/*   Updated: 2022/10/31 22:16:28 by bmugnol-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_config	*get_config(int argc, char **argv)
{
	...
	config->time_to_die_usec = ft_atoi(argv[2]) * (long)(MILLI_TO_MICRO);
	config->time_to_eat_usec = ft_atoi(argv[3]) * (long)(MILLI_TO_MICRO);
	config->time_to_sleep_usec = ft_atoi(argv[4]) * (long)(MILLI_TO_MICRO);
	if (argv[4])
